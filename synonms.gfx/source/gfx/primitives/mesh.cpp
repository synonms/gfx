...
Mesh::Mesh(const VertexDefinition& vertexDefinition, float* vertexData, unsigned int noOfVertexBytes, unsigned int* indexData, unsigned int noOfIndexBytes)
    : _noOfVertexBytes(noOfVertexBytes)
    , _noOfIndexBytes(noOfIndexBytes)
{
    ...
    proxies::opengl::VertexBuffer::SendData(vertexData, noOfVertexBytes, proxies::opengl::enumerators::DataUsage::StaticDraw, true);

    auto vertexSize = vertexDefinition.Size();
    ...
    proxies::opengl::IndexBuffer::SendData(indexData, noOfIndexBytes, proxies::opengl::enumerators::DataUsage::StaticDraw, true);
}
...
void Mesh::Draw()
{
    ...
    proxies::opengl::IndexBuffer::Bind(_indexBufferId, true);
    proxies::opengl::IndexBuffer::Draw(proxies::opengl::enumerators::DrawMode::Triangles, _noOfIndexBytes, proxies::opengl::enumerators::DataType::UnsignedInt, true);
}

std::string Mesh::ToString()
{
    std::stringstream stream;
    stream << "[Mesh: VertexArrayId " << _vertexArrayId << ", VertexBufferId " << _vertexBufferId << ", IndexBufferId " << _indexBufferId << ", " << _noOfVertexBytes << " positions, " << _noOfIndexBytes << " indices]";
